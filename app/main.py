...
logger = logging.getLogger("virchow_pipeline")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Virchow2 feature extraction pipeline for RunPod",
...
def run(cfg: JobConfig, args: argparse.Namespace) -> int:
    ...
    logger.info("=" * 60)
    logger.info("Virchow2 Feature Extraction Pipeline")
    ...
    # Weights are either downloaded dynamically by HuggingFace Hub during 
    # extract_features() or pre-cached by download_models.sh
    ...
    # ── 6. Extract features ───────────────────────────────────────────────────
    logger.info("Stage 6: Extracting features with Virchow2 …")
    total_patches = 0
    total_failed = 0

    try:
        results = extract_features(str(slide_input_dir))
        
        features = results["embeddings"]
        names = results["patch_names"]
        total_patches = results["patch_count"]
        ...
        if total_patches > 0:
            ...
            with FeatureWriter(
                ...
                feature_dim=results["embedding_dim"],
            ) as writer:
                writer.write_batch(features, names, coords)
        else:
            logger.info("No patches found or extracted.")
    ...
    model_name = getattr(cfg, "ai_model_name", None) or "Virchow2"
